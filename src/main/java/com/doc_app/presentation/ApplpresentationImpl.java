package com.doc_app.presentation;
...
import com.doc_app.helper.*;
import com.doc_app.service.AdminService;
import com.doc_app.service.DoctorServiceImpl;
import com.doc_app.service.DoctorService;
import com.doc_app.service.PatientService;


@Component("docAppPresentation")
public class ApplpresentationImpl implements Applpresentation {
	static Scanner sc=new Scanner(System.in);
	
	@Autowired
	private AdminService adminServiceInterface;
	public void setAdminServiceInterface(AdminService adminServiceInterface) {
		this.adminServiceInterface = adminServiceInterface;
	}

	@Autowired
	private DoctorService doctorServiceInterface;
	public void setDoctorServiceInterface(DoctorService doctorServiceInterface) {
		this.doctorServiceInterface = doctorServiceInterface;
	}
	@Autowired
	private PatientService patientServiceInterface;
	public void setPatientServiceInterface(PatientService patientServiceInterface) {
...
//			doccid=dID;
			...
			System.out.println();
			loop2=adminServiceInterface.adminLogin(aID,aPassword);
			if(loop2) {
			...
			while(loop2) {
				...
				switch(c2) {
					case 1:
						...
						System.out.println("+---------+-----------+---------+--------+");
						ArrayList<Availability>availbydid=adminServiceInterface.availbydoc(did);
						...
						System.out.println("+--------------+----------+---------+----------+---------+");
						appointments=adminServiceInterface.view_appointments(did);
					...
					case 2:
						...
						int ad=sc.nextInt();
						System.out.println();
						boolean flag= adminServiceInterface.cancel_appointments(ad);
						if (flag==true)
					...
					case 3:
						...
						System.out.println();
						flag=adminServiceInterface.modify_appointments(ad1,did1);
						if (flag==true)
...
//			doccid=dID;
			...
			dID=doctorServiceInterface.doctorLogin(dID,dPassword);
			if (dID!=-1) {		
			  ...
			  while(loop2) {
				...
				switch(choice) {
					...
					case 1:
						...
						System.out.println("|Appointment_Id|Doctor_Id|Patient_Id|Date      |Time     |");
						System.out.println("+--------------+----------+---------+----------+---------+");
						ArrayList<Appointment> appointments= doctorServiceInterface.view_appointments(dID);
						for(Appointment appointment:appointments) {
							System.out.println("|"+appointment.getaID()+"             |"+appointment.getDoctorId()+ "         |"+appointment.getPatientID()+ "      |"+appointment.getaDate()+"|"+appointment.getaTime()+" |");
							System.out.println("+--------------+----------+---------+----------+---------+\n");
						}
...
//						String dname = doccname;
						...
						boolean flag= doctorServiceInterface.set_availability(availability);
						if(flag==true) {
					...
					case 3:
						...
						int aid=sc.nextInt();
						boolean check= doctorServiceInterface.cancel_appointments(dID,aid);
						if (check==true) {
							System.out.println("Cancelled!");
						}
						else {
		...
		case 1:
			boolean flag=true;
			while(flag) {
				DoctorServiceImpl doctorService1=new DoctorServiceImpl();
...
//			doccid=dID;
			...
			System.out.println();
			int pid=patientServiceInterface.patientLogin(pEmail,pPassword);
			if (pid!=-1) {				
			  ...
			  while(loop2) {
				...
				switch(choice) {
					case 1:
						...
						ArrayList<Appointment>appointments=patientServiceInterface.view_appointments(pid);
						for(Appointment appointment:appointments) {
							System.out.println("|"+appointment.getaID()+"             |"+appointment.getDoctorId()+ "         |"+appointment.getPatientID()+ "      |"+appointment.getaDate()+"|"+appointment.getaTime()+" |");
							System.out.println("+--------------+----------+---------+----------+---------+\n");
						}
					...
					case 2:
						...
						flag=patientServiceInterface.add_appointment(appointment);
						if(flag==true) {
					...
					case 3:
						System.out.println("Enter the appointment Id:");
						int aid=sc.nextInt();
						boolean check= patientServiceInterface.cancel_appointments(pid,aid);
						if (check==true) {
							System.out.println("Cancelled!");
						}
						else {
		...
		case 1:
			...
			while(flag) {	    	
				...
				flag=patientServiceInterface.patientRegister(patient);
				if (flag==true)
					System.out.println("Not possible!!");
				else {
					System.out.println("Registered!");
	...
	public void performmenu() throws Exception {
    ...
    public void menuoptdoc() {
    	System.out.println("**Enter your option:\n1) View Appointments  \n2) Set Availability \n3) Cancel Appointment\n4) Exit");
    	System.out.print("Enter your choice: ");
    	System.out.println();
    }
    
    public void menuoptpat() {
		System.out.println("Enter the option:");
